...
from utils.vector_store import VectorStatus
...
def check_file_status(file_id: str, table_name: str):
    ...
    with Session() as session:
        ...
        if all(doc.status == VectorStatus.SUCCESS for doc in docs):
            new_status = FileStatus.SUCCESS
        elif any(doc.status == VectorStatus.FAILED for doc in docs):
...
def embed_doc(
    ...
    with Session() as session:
        try:
            ...
            doc.status = VectorStatus.SUCCESS
            session.commit()
        except Exception:
            doc.status = VectorStatus.FAILED
            session.commit()
            raise
